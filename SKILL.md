...
Default to the smallest API-key route that matches the task:
...
| Long-running or high-resolution task | `job run` | `POST /v1/images/jobs` then `GET /v1/images/jobs/:id` |
...
Async high-resolution job:

```powershell
uv run codex2api-image job run `
  --prompt "Improve sharpness only; preserve layout and identity" `
  --image G:\images\source.png `
...
- `references/prompting.md` for minimal prompt handling and background wording.
